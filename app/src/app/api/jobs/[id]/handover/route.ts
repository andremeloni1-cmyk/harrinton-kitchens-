...
import { finalInvoiceLines, finalInvoiceBalanceCents, CARE_INSTRUCTIONS, WARRANTY_TERMS } from "@/lib/handover";
...
export async function POST(req: Request, { params }: Params) {
  ...
  const [settings, drawingSets, snags, variations, invoices] = await Promise.all([
    ...
    prisma.invoice.findMany({ where: { jobId }, select: { amountPaid: true } }),
  ]);
  ...
  const paidIncCents = Math.round(invoices.reduce((s, i) => s + (i.amountPaid || 0), 0) * 100);
  const estimate = parseLineItems(job.estimateItems ?? "[]");
  const baseLines: LineItem[] = estimate.length > 0
    ? estimate
    : [{ description: [`${job.reference} — ${job.title}`, job.address].filter(Boolean).join(" · "), quantity: 1, unitAmount: job.quoteAmount ?? 0 }];
  const baseSubtotalCents = Math.round(baseLines.reduce((s, l) => s + l.quantity * l.unitAmount, 0) * 100);
  const finalLines = finalInvoiceLines(baseLines, variations, paidIncCents);
  ...
  const balanceCents = finalInvoiceBalanceCents(baseSubtotalCents, variations, paidIncCents);
