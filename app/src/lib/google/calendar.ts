...
function buildDescription(job: JobLike, docLinks: string[] = []): string {
  ...
  lines.push("", `Managed by Harrington Kitchens • ${process.env.APP_URL || ""}/jobs/${job.id}`);
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const res = await calendar.events.insert({
      ...
      requestBody: {
        ...
        extendedProperties: { private: { harringtonkitchensJobId: job.id } },
      },
    });
...
export async function listEvents(timeMin: Date, timeMax: Date): Promise<ExternalEvent[] | null> {
  ...
  return (res.data.items || [])
    ...
    .filter((e) => !e.extendedProperties?.private?.harringtonkitchensJobId)
    .map((e) => ({
      id: e.id || "",
...
/**
 * Reads back the events this app created (tagged with harringtonkitchensJobId), returning
...
export async function listJobEventStarts(): Promise<Map<string, { start: Date; allDay: boolean }>> {
  ...
  for (const e of res.data.items || []) {
    ...
    const jobId = e.extendedProperties?.private?.harringtonkitchensJobId;
